...
function startRecommendedProgram(programId) {
  ...
  // Navigate to programs and open the recommended one
  ...
  setTimeout(function() {
    const allProgs = typeof allProgs === 'function' ? allProgs() : [];
    const prog = allProgs.find(function(p) {
      return p.id === programId || 
             (p.name && p.name.toLowerCase().includes(programId.replace('-builtin', '').replace('kinetiq-', '')));
    });
    
    if (prog && typeof openProg === 'function') {
      openProg(prog);
    }
  }, 100);
}
