...
var FATSECRET_CLIENT_ID = '34ebfa6431064439882f1a948a4e053a';
var FATSECRET_CLIENT_SECRET = '758888dfc68b4411b60b381cb2810bcb';
var fatSecretAccessToken = null;
...
async function getFatSecretToken() {
  ...
  if (fatSecretAccessToken && tokenExpiry && Date.now() < tokenExpiry) {
    return fatSecretAccessToken;
  }
  
  // Get new token using OAuth 2.0
  try {
    var credentials = btoa(FATSECRET_CLIENT_ID + ':' + FATSECRET_CLIENT_SECRET);
    var response = await fetch('https://oauth.fatsecret.com/connect/token', {
      method: 'POST',
      headers: {
...
async function searchFatSecret() {
  ...
  resultsDiv.innerHTML = '<div style="text-align: center; padding: 20px; color: var(--t2);">🔍 Searching 900,000+ foods...</div>';
  
  try {
    var token = await getFatSecretToken();
    if (!token) {
      resultsDiv.innerHTML = '<div class="empty" style="padding: 20px;"><div class="esub">Connection error. Please try again.</div></div>';
      return;
    }
    
    var response = await fetch('https://platform.fatsecret.com/rest/server.api?' + 
      'method=foods.search&' +
    ...
    resultsDiv.innerHTML = foods.map(function(food) {
      // Parse description for display
      var desc = food.food_description || '';
      var shortDesc = desc.length > 80 ? desc.substring(0, 77) + '...' : desc;
      
      return '<div class="food-result-item">'
        + '<div style="flex: 1;"><div class="food-result-name">' + esc(food.food_name) + '</div>'
        + '<div class="food-result-macros">' + esc(shortDesc) + '</div></div>'
        + '<button class="food-result-add" onclick="addFatSecretFood(\'' + food.food_id + '\')">+ Add</button>'
        + '</div>';
    }).join('');
...
async function addFatSecretFood(foodId) {
  try {
    var token = await getFatSecretToken();
    if (!token) {
    ...
    if (!food || !food.servings || !food.servings.serving) {
      alert('Could not get food details');
      return;
    }
    
    // Get first serving
    var serving = Array.isArray(food.servings.serving) ? food.servings.serving[0] : food.servings.serving;
    
    var entry = {
      id: uid(),
      name: food.food_name + ' (' + (serving.serving_description || '1 serving') + ')',
      cals: parseInt(serving.calories) || 0,
...
function searchFoods() {
  ...
  results.innerHTML = matches.map(function(food) {
    return '<div class="food-result-item">'
      + '<div><div class="food-result-name">' + esc(food.name) + '</div>'
