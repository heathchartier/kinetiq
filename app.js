...
function showBarcodeScanner() {
  ...
  document.getElementById('barcode-scanner-section').style.display = 'block';
  ...
  document.getElementById('scan-btn').classList.remove('btn-s');
  document.getElementById('scan-btn').classList.add('btn-p');
...
async function startBarcodeScanner() {
  ...
  resultDiv.innerHTML = '<div style="text-align: center; padding: 12px; color: var(--t2); font-size: 13px;">📱 Starting camera...</div>';
  
  try {
    // Use native getUserMedia API
    ...
    resultDiv.innerHTML = '<div style="padding: 12px;">'
      + '<div style="text-align: center; color: var(--acc); font-weight: 700; margin-bottom: 12px;">📱 Camera Active</div>'
      + '<div style="font-size: 12px; color: var(--t2); margin-bottom: 12px; text-align: center;">Manual entry option:</div>'
      + '<input type="number" class="fi" id="manual-barcode" placeholder="Enter barcode number" style="margin-bottom: 8px;">'
      + '<button class="btn-p" onclick="lookupManualBarcode()" style="width: 100%;">Lookup Barcode</button>'
      + '</div>';
    ...
    resultDiv.innerHTML = '<div style="padding: 12px;">'
      + '<div style="text-align: center; color: var(--danger); font-size: 13px; margin-bottom: 12px;">⚠️ Camera not available</div>'
      + '<div style="font-size: 12px; color: var(--t2); margin-bottom: 12px; text-align: center;">Enter barcode manually:</div>'
      + '<input type="number" class="fi" id="manual-barcode" placeholder="Enter barcode number (e.g., 016000275447)" style="margin-bottom: 8px;">'
      + '<button class="btn-p" onclick="lookupManualBarcode()" style="width: 100%;">Lookup Barcode</button>'
      + '<div style="font-size: 11px; color: var(--t3); margin-top: 8px; text-align: center;">Barcode is usually 12-13 digits on package</div>'
      + '</div>';
...
function stopBarcodeScanner() {
  ...
  document.getElementById('barcode-scanner-section').style.display = 'none';
  
  // Show Quick Add by default
  showQuickAdd();
}
...
async function lookupBarcode(barcode) {
  var resultDiv = document.getElementById('barcode-result');
  resultDiv.innerHTML = '<div style="text-align: center; padding: 12px; color: var(--t2); font-size: 13px;">🔍 Looking up barcode...</div>';
  
  try {
    // Search USDA by barcode (gtin_upc field)
    var response = await fetch(USDA_API_URL + '/foods/search?api_key=' + USDA_API_KEY + '&query=' + barcode + '&dataType=Branded&pageSize=5');
    var data = await response.json();
    
    if (!data.foods || data.foods.length === 0) {
      resultDiv.innerHTML = '<div style="padding: 12px; text-align: center;">'
        + '<div style="color: var(--t1); font-weight: 700; margin-bottom: 4px;">Barcode: ' + barcode + '</div>'
        + '<div style="color: var(--t3); font-size: 13px;">Not found in database</div>'
        + '<button class="btn-s btn-sm" onclick="showBarcodeScanner()" style="margin-top: 12px;">Scan Another</button>'
        + '</div>';
      return;
    }
    
    // Show results
    resultDiv.innerHTML = '<div style="margin-bottom: 8px; font-weight: 700; color: var(--t1); font-size: 13px;">Barcode: ' + barcode + '</div>'
      + data.foods.slice(0, 3).map(function(food) {
        ...
        return '<div class="food-result-item" style="margin-bottom: 8px;">'
      ...
      + '<button class="btn-s btn-sm" onclick="showBarcodeScanner()" style="width: 100%; margin-top: 8px;">Scan Another</button>';
    
  } catch (error) {
    console.error('Barcode lookup error:', error);
    resultDiv.innerHTML = '<div style="padding: 12px; color: var(--danger); text-align: center; font-size: 13px;">Error looking up barcode. Please try again.</div>'
      + '<button class="btn-s btn-sm" onclick="showBarcodeScanner()" style="margin-top: 12px;">Try Again</button>';
  }
}
