...
async function startBarcodeScanner() {
  ...
  resultDiv.innerHTML = '<div style="text-align: center; padding: 12px; color: var(--acc); font-size: 13px; font-weight: 700;">📱 Point camera at barcode...<br><span style="font-size: 11px; color: var(--t3); font-weight: 400; margin-top: 4px; display: block;">Will auto-detect and lookup</span></div>';
  ...
  try {
    ...
    resultDiv.innerHTML = '<div style="padding: 12px; color: var(--danger); text-align: center; font-size: 13px;">⚠️ Camera access denied or unavailable<br><br>'
      + '<button class="btn-s btn-sm" onclick="stopBarcodeScanner()" style="margin-top: 8px;">Go Back</button></div>';
  }
}
...
async function lookupBarcode(barcode) {
  var resultDiv = document.getElementById('barcode-result');
  resultDiv.innerHTML = '<div style="text-align: center; padding: 12px; color: var(--t2); font-size: 13px;">🔍 Looking up barcode ' + barcode + '...</div>';
  
  try {
    // Search USDA by barcode (gtin_upc field)
    var response = await fetch(USDA_API_URL + '/foods/search?api_key=' + USDA_API_KEY + '&query=' + barcode + '&dataType=Branded&pageSize=10');
    var data = await response.json();
    
    if (!data.foods || data.foods.length === 0) {
      resultDiv.innerHTML = '<div style="padding: 16px; text-align: center;">'
        + '<div style="color: var(--t1); font-weight: 700; margin-bottom: 8px;">Barcode: ' + barcode + '</div>'
        + '<div style="color: var(--t3); font-size: 13px; margin-bottom: 12px;">Not found in USDA database</div>'
        + '<div style="font-size: 11px; color: var(--t2); margin-bottom: 12px;">Try searching by food name instead, or this item may not be in the database yet.</div>'
        + '<div style="display: flex; gap: 8px;">'
        + '<button class="btn-s" onclick="showQuickAdd()" style="flex: 1; font-size: 11px;">Search Foods</button>'
        + '<button class="btn-p" onclick="showBarcodeScanner()" style="flex: 1; font-size: 11px;">Scan Another</button>'
        + '</div></div>';
      return;
    }
    
    // Show results
    resultDiv.innerHTML = '<div style="margin-bottom: 12px;"><div style="font-weight: 700; color: var(--ok); font-size: 13px; margin-bottom: 8px;">✓ Found!</div></div>'
      ...
      + '<button class="btn-p btn-sm" onclick="showBarcodeScanner()" style="width: 100%; margin-top: 8px;">Scan Another</button>';
    
  } catch (error) {
    console.error('Barcode lookup error:', error);
    resultDiv.innerHTML = '<div style="padding: 16px; text-align: center; color: var(--danger); font-size: 13px;">Error looking up barcode. Please try again.</div>'
      + '<button class="btn-p btn-sm" onclick="showBarcodeScanner()" style="margin-top: 12px; width: 100%;">Try Again</button>';
  }
}
