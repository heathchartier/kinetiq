...
function calcTDEE() {
  ...
  // REALISTIC MACRO SPLIT:
  // Protein: 1g per lb bodyweight (industry standard)
  // Then split remaining calories between carbs and fat based on goal
  var proteinGrams = Math.round(weight); // 1g per lb bodyweight
  var proteinCals = proteinGrams * 4;
  
  var remainingCals = target - proteinCals;
  
  // Macro ratios for remaining calories (after protein is set)
  var carbRatio, fatRatio;
  if (goal === 'cut') {
    // Cut: Lower carbs, moderate fat (45% carb, 30% fat of remaining cals)
    carbRatio = 0.60;
    fatRatio = 0.40;
  } else if (goal === 'bulk') {
    // Bulk: Higher carbs for energy (70% carb, 30% fat of remaining cals)
    carbRatio = 0.70;
    fatRatio = 0.30;
  } else {
    // Maintain: Balanced (65% carb, 35% fat of remaining cals)
    carbRatio = 0.65;
    fatRatio = 0.35;
  }
  
  var carbCals = remainingCals * carbRatio;
  var fatCals = remainingCals * fatRatio;
  
  var carbGrams = Math.round(carbCals / 4);
  var fatGrams = Math.round(fatCals / 9);
  
  // Calculate actual percentages for display
  var proteinPct = Math.round((proteinCals / target) * 100);
