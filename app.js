...
var USDA_API_KEY = 'DEMO_KEY'; // Free tier, no signup needed
...
function searchFoods() {
  ...
  results.innerHTML = matches.map(function(food) {
    ...
    return '<div class="food-result-item">'
      ...
      + '<button class="food-log-delete" onclick="toggleFavorite(\'' + esc(food.name.replace(/'/g, "\\'")) + '\',' + food.cals + ',' + food.protein + ',' + food.carbs + ',' + food.fat + '); searchFoods();" style="' + (isFav ? 'background: var(--acc); color: #000; border-color: var(--acc);' : '') + '">★</button>'
      + '</div></div>';
  }).join('');
}
...
function renderFavorites() {
  ...
  favList.innerHTML = favoriteFoods.map(function(f) {
    return '<div class="food-result-item">'
      ...
      + '<button class="food-log-delete" onclick="toggleFavorite(\'' + esc(f.name.replace(/'/g, "\\'")) + '\',' + f.cals + ',' + f.protein + ',' + f.carbs + ',' + f.fat + ')">★</button>'
      + '</div></div>';
  }).join('');
}
